...
# --- Custom EB Garamond + Robinhood legend CSS for perfect centering, white text, soft shadows ---
st.markdown("""
    <style>
...
with st.container():
    ...
    # Calculate robust metrics (bulletproof for all edge cases)
    close_series = df["Close"].dropna()
    if len(close_series) == 0:
        cur_price = prev_price = pct_chg = 0.0
    elif len(close_series) == 1:
        cur_price = prev_price = close_series.iloc[0]
        pct_chg = 0.0
    else:
        cur_price = float(close_series.iloc[-1])
        prev_price = float(close_series.iloc[-2])
        if prev_price != 0 and not pd.isnull(prev_price):
            pct_chg = ((cur_price - prev_price) / prev_price) * 100
        else:
            pct_chg = 0.0

    spark_df = close_series.tail(30)
